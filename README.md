...
**File → Export frame PNG… / Export all PNGs…** lets you choose any project language without changing the preview. Each PNG is indexed (color type 3) with exactly the distinct RGB colors visible in that image, merging duplicate colors and dropping unused palette entries. Text edges use a small set of blended shades for smoother letters; selection markers are excluded. Each frame gets its own minimal palette and the smallest supported PNG bit depth. The editable project palette stays unchanged.
