...
Opening an associated project adds a project tab. An already open project switches to its tab. Each tab keeps its own artwork, undo history, and save target; all open tabs are included in crash recovery. Compiled `.cutscene.runtime` files are display assets, not editable projects.

The palette editor and individual color dialog use [AvaloniaColorPicker](https://github.com/arklumpus/AvaloniaColorPicker) with a local Avalonia 12 compatibility port. Indexed projects retain a reserved transparent index; RGBA projects add editable swatch alpha. Its LGPL license, complete source, and rebuild instructions are in [third_party/AvaloniaColorPicker](third_party/AvaloniaColorPicker/README.comic-editor.md), the release source archive, and **Help > About & licenses**. ComicEditor's own software remains 0BSD.
...
The vertical Material icon rail groups related tools: brushes (pixel, smooth, pressure, marker), lines (line, curve), shapes (rectangle, rounded rectangle, ellipse, polygon), spray/dither, selections (rectangle, freehand, ellipse), and effects (pixel scramble, blur). Right-click a grouped icon to choose a variant; left-click selects the last variant used in that group. The compact tool picker shows the same groups. **Tool options** (the sliders icon on desktop) sets the brush size, tip shape, shape fill, or spray density. Brush tips include round, square, two diagonal tips, horizontal, and vertical. Hold the spray can still to build up paint. Curves start with a dragged line followed by two dragged bends. Polygons accept clicked corners; double-click, Enter, or Finish closes them. Escape cancels an unfinished path.
...
**Edit → Drawing input…** enables mouse/touchpad smoothing for freehand tools. Pen input retains its pressure response; the Smooth brush also smooths pen strokes. **Scroll over the canvas** to zoom at the pointer (no modifier needed), or **pinch with two fingers**. **Middle-button drag**, **two-finger drag**, or **three-finger drag** pans the zoomed canvas. Single-finger touch still draws; adding a second finger cancels the provisional mark without changing undo/redo history. Lift all fingers before drawing again. The zoom selector offers Fit and fixed zoom levels. The Zoom tool uses click to enlarge and right-click or Shift-click to reduce. Indexed artwork uses nearest-neighbor bitmap rendering to avoid seams at fractional zoom.

New text reuses the last applied font, size, bold, and italic style. New projects reuse the latest canvas dimensions and palette. The active color/tool, last variant in each tool group, each tool's size and brush shape, spray density, shape fill, smoothing, preview language, onion skin, comparison, and zoom are remembered across sessions. These preferences are separate from cutscene files, stored in the application's local data directory (`ComicEditor/preferences.json`) or browser local storage.
...
Each cutscene has its own palette (128 swatches by default). New indexed projects retain 2–255 opaque colors and index 255 for transparency. Choose **File → New RGBA cutscene** or **Canvas → Convert to RGBA color…** for translucent swatches, source-over painting, and larger palettes; **File → New indexed cutscene** remains available. The marker brush defaults to partial opacity and a larger size. Dither paints a repeating Bayer pattern, pixel scramble shuffles nearby artwork, and blur softens neighboring pixels in either color mode. Open **Palette → Palette editor…** to edit colors and alpha or import/export a GIMP `.gpl` file. Pressure uses normal pen/pointer pressure, falling back to full pressure for devices without it.
...
Choose **Text**, then **click and drag** to create its wrapping area and type directly into it. Double-click existing text, press F2 after selecting it, or use **Edit on canvas** to edit the current preview language in place. The inline toolbar offers font, size, and Style controls, including on mobile. Select a text range and press **Ctrl+>** or **Ctrl+<** (Ctrl+Shift+period/comma), or use A+/A−, to change that range's size. Dragging an existing object moves it, and its eight handles resize the area for the preview language; other languages retain their own placement. **Delete** removes selected text when focus is outside an input field. The inspector also edits the selected language; **Layout & style…** opens a modal for position, dimensions, font, size, style, and palette color. Missing and overflowing translations are marked in the editor. Exported artwork never contains localization keys or selection markers.
...
**File → Export frame PNG… / Export all PNGs…** lets you choose any project language without changing the preview. Indexed projects export indexed PNGs (color type 3) with exactly the distinct RGB colors visible in each image. RGBA projects export lossless truecolor PNGs with alpha (color type 6), preserving mixed colors and transparency. The editable project palette stays unchanged.

**File → Export PDF… / Export EPUB… / Export CBZ…** creates one page or image per frame in storyboard order, using the language selected in the export dialog. PDF embeds frame images; EPUB is a fixed-layout image book; CBZ contains numbered PNGs. Text is baked into these reading copies, so export each desired language separately. Keep the `.ctsc` project for editable text and translations. EPUB pages use images, so their text is not selectable or searchable in a reader.

## Palette presets

The palette editor works on a draft. **Apply** updates only the current cutscene; **Cancel** discards the draft. Indexed artwork and text keep their slot numbers when the palette is recolored. Shrinking remaps removed indexed slots to the nearest remaining RGB color. RGBA artwork keeps its painted pixels; text keeps its palette references. Imported palettes must have at least two colors. GIMP palette files remain RGB compatible; ComicEditor preserves optional alpha in comments.
...
A cutscene embeds its complete palette: modifying it never rewrites a saved preset, and it renders without access to the palette folder. The [.gpl specification](https://developer.gimp.org/core/standards/gpl/) defines the portable RGB text format. Indexed files use editable format version 3 and compiled version 2; RGBA files use editable version 4 and compiled version 3. See the [runtime format guide](docs/runtime-format.md).
