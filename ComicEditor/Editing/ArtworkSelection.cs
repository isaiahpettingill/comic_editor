...
public sealed class ArtworkSelection
{
    ...
    public ArtworkSelection(ArtworkLayer layer, int x, int y, int width, int height, IReadOnlyList<(double X, double Y)>? polygon = null)
    {
        ...
        Pixels = new int[width * height]; Mask = new bool[Pixels.Length];
        for (var py = 0; py < height; py++)
            for (var px = 0; px < width; px++)
            {
                var i = py * width + px;
                Mask[i] = polygon is null || PaintRaster.Inside(polygon, x + px + .5, y + py + .5);
                Pixels[i] = layer.Pixel(x + px, y + py);
            }
    }
    private ArtworkSelection(ArtworkSelection other, ArtworkLayer owner)
    { Owner = owner; X = other.X; Y = other.Y; Width = other.Width; Height = other.Height; Pixels = (int[])other.Pixels.Clone(); Mask = (bool[])other.Mask.Clone(); }
